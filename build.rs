...
fn list_active_features() {
    ...
    FEATURE_LIST.iter().filter(|f| f.is_active()).for_each(|f| println!("{}", f));
}

fn list_removed_features() {
    println!("These features are DEPRECATED and will never be compiled");
    FEATURE_LIST.iter().filter(|f| f.was_removed()).for_each(|f| println!("{}", f));
}

fn resolve_features(target: Target) -> Result<(), String> {
    match target {
        Target::MainNet => { /* No features are active at all */ },
        Target::NextNet => FEATURE_LIST.iter().filter(|f| f.is_active_in_nextnet()).for_each(|f| activate_feature(f)),
        Target::Dibbler => FEATURE_LIST.iter().filter(|f| f.is_active_in_dibbler()).for_each(|f| activate_feature(f)),
    }
    Ok(())
}
